...
function convert_parent_to_second_sibling_histogram!(parent_histogram, sibling_histogram)
  epsilons = map(eps, parent_histogram)

  parent_histogram .-= sibling_histogram

  for i in 1:length(parent_histogram)
    if parent_histogram[i] != 0f0 && abs(parent_histogram[i]) <= epsilons[i]
      println(parent_histogram)
